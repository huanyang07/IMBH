...
class ConservativeAffineMacroAtlas:
    ...
    macro_coordinate_pullback: np.ndarray | None = None
    ...
    def evaluate(self, macro_state) -> ConservativeMacroOutputs:
        ...
        if self.macro_coordinate_pullback is not None:
            ...
            coordinate = np.einsum("kij,kj->ki", pullback, coordinate)
...
def thermodynamic_chart_lift(context, anchor_primitive_charts, chart_coordinates) -> np.ndarray:
    """Generate an admissible hydrostatic truth state from 16 local charts.
