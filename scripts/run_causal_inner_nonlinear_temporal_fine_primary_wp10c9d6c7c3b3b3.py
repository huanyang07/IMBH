#!/usr/bin/env python3
"""Run the frozen fine-layout primary nonlinear temporal confirmation."""
...
ANALYZED_BASE_COMMIT = "52fceff69c7c684a0601ad696c6a81d109c5a1a1"
ANALYZED_BASE_PARENT = "f8d9b1f20eae1c90fff77d4903d0c9179f676652"
ANALYZED_BASE_TREE = "502542827a5354f963c7111ebf3442b7fae0dab1"

LAYOUT = c3b3a.FINE_LAYOUT
...
THIS_RUNNER = (
    "scripts/run_causal_inner_nonlinear_temporal_fine_primary_"
    "wp10c9d6c7c3b3b3.py"
)
THIS_TEST = (
    "tests/test_causal_inner_nonlinear_temporal_fine_primary_"
    "wp10c9d6c7c3b3b3.py"
)
REPORT_RELATIVE = (
    "docs/reports/current/"
    "CODEX_CAUSAL_INNER_NONLINEAR_TEMPORAL_FINE_PRIMARY_"
    "WP10C9D6C7C3B3B3_2026-08-02.md"
)
...
def _validate_parent() -> tuple[dict, dict]:
    ...
    if (
        ...
        or not parent_screen[
            "fine_primary_temporal_confirmation_authorized"
        ]
        ...
        raise RuntimeError("c3b3b2 fine-confirmation authorization changed")
    if (
        ...
        raise RuntimeError("c3b3b3 analyzed identity changed")
...
def _trajectory(
    ...
    for index in range(step_count):
        ...
        print(
            f"c3b3b3: {trajectory_id} dt={timestep:.1e} "
...
def _export_history(context, states: np.ndarray) -> tuple[np.ndarray, dict]:
    ...
    for index, state in enumerate(states):
        ...
        print(
            f"c3b3b3: direct export {index + 1}/{len(states)} "
...
def _load_progress() -> tuple[dict, dict[str, np.ndarray]]:
    ...
    if (
        ...
        raise RuntimeError("saved c3b3b3 progress belongs to different code")
...
def _report(summary: dict) -> str:
    lines = [
        "# Nonlinear fine-layout primary temporal confirmation "
        "WP10c9d6c7c3b3b3",
        "",
        "## Classification",
        "",
        ...
        "",
        "The unchanged fine embedded operator was compared at "
...
def _package(
    ...
    classification = (
        ...
        if passed
        else "fine_primary_nonlinear_temporal_confirmation_failed_"
        "duration_extension_blocked"
    )
    authorized_next = (
        "WP10c9d6c7c3b3b4_coarse_primary_nonlinear_symmetry_controls"
        if passed
        else "WP10c9d6c7c3b3b3_temporal_failure_localization"
    )
    gates = _temporal_gates(manifest)
    config = {
        "schema_version": SCHEMA_VERSION,
    ...
    summary = {
        ...
        "coarse_primary_nonlinear_symmetry_controls_authorized": passed,
    ...
    input_paths = {
        "parent_temporal_confirmation_summary": PARENT_SCREEN_DIRECTORY
        / "summary.json",
...
def main() -> int:
    ...
    print(f"c3b3b3: build tangent {LAYOUT}", flush=True)
