#!/usr/bin/env python3
"""Run the frozen coarse held-out nonlinear temporal-refinement stage."""
...
ANALYZED_BASE_COMMIT = "abb848d08c181f77b477575baba116c823569d6f"
ANALYZED_BASE_PARENT = "0e3a6feca400b386d9fdb47754aa4b58460cfea3"
ANALYZED_BASE_TREE = "6c089b97fdbe85d6f3ae41ba7f0c8b6d3dbc35bc"

LAYOUT = c3b4a.LAYOUTS[0]
PROFILES = tuple(c3b4a.PROFILE_NAMES)
TIMESTEP_LEVELS_SECONDS = np.asarray(c3b4a.TIMESTEP_LEVELS_SECONDS, dtype=float)
REFINED_TIMESTEPS_SECONDS = TIMESTEP_LEVELS_SECONDS[1:]
HORIZON_SECONDS = c3b4a.HORIZON_SECONDS
COMMON_OUTPUT_TIMES_SECONDS = np.asarray(c3b4a.COMMON_OUTPUT_TIMES_SECONDS)
...
THIS_TEST = (
    "tests/test_causal_inner_nonlinear_profile_breadth_temporal_"
    "wp10c9d6c7c3b4b2.py"
)
REPORT_RELATIVE = (
    "docs/reports/current/"
    "CODEX_CAUSAL_INNER_NONLINEAR_PROFILE_BREADTH_TEMPORAL_"
    "WP10C9D6C7C3B4B2_2026-08-02.md"
)
REPORT_PATH = ROOT / REPORT_RELATIVE

PARENT_DIRECTORY = c3b4b1.CANONICAL_DIRECTORY
COARSE_TEMPORAL_DIRECTORY = c3b3b1.CANONICAL_DIRECTORY
TEMPORAL_MANIFEST_DIRECTORY = c3b3a.CANONICAL_DIRECTORY
...
def _source_identity() -> dict[str, str]:
    paths = (THIS_RUNNER, THIS_TEST, c3b4b1.THIS_RUNNER, c3b3b1.THIS_RUNNER)
    return {
...
def _validate_parent() -> tuple[dict, dict, dict]:
    ...
    temporal_manifest = _read_json(
        TEMPORAL_MANIFEST_DIRECTORY / "temporal_refinement_manifest.json"
    )
    if (
        not parent["passed"]
        or parent["authorized_next"]
        != "WP10c9d6c7c3b4b2_coarse_heldout_profile_temporal_refinement"
        or not parent["coarse_heldout_temporal_refinement_authorized"]
        ...
        raise RuntimeError("b4b1 temporal authorization changed")
    stage = breadth_manifest["campaign_controller"]["stages"][1]
    if (
        stage["work_package"] != WORK_PACKAGE
        or stage["layout"] != LAYOUT
        or tuple(stage["profiles"]) != PROFILES
        or not np.array_equal(stage["timestep_seconds"], REFINED_TIMESTEPS_SECONDS)
        or not stage["stop_on_any_state_or_Tier_I_temporal_failure"]
    ):
        raise RuntimeError("b4a coarse temporal stage changed")
    if temporal_manifest["classification"] != (
        "nonlinear_temporal_refinement_manifest_frozen_"
        "coarse_temporal_screen_authorized"
    ):
        raise RuntimeError("temporal contract changed")
    if (
        ...
        raise RuntimeError("b4b2 analyzed identity changed")
    return parent, breadth_manifest, temporal_manifest


def _task_id(timestep: float, profile: str) -> str:
    token = f"{float(timestep):.1e}".replace(".", "p").replace("-", "m")
    return f"{profile}__p1__dt_{token}"
...
def _load_progress() -> tuple[dict, dict[str, np.ndarray]]:
    ...
    if (
        ...
        raise RuntimeError("saved b4b2 progress belongs to different code")
...
def _run_refined(configuration: dict, tangent) -> tuple[dict, dict[str, np.ndarray]]:
    ...
    try:
        for timestep in REFINED_TIMESTEPS_SECONDS:
            for profile in PROFILES:
                task = _task_id(float(timestep), profile)
...
def _common_indices(timestep: float) -> np.ndarray:
    return c3b3b1._common_indices(float(timestep))


def _h_base_exports(configuration: dict) -> np.ndarray:
    base_arrays = _load_npz(BASE_DIRECTORY / "decisive_arrays.npz")
    states = np.asarray(base_arrays[f"{LAYOUT}__states"], dtype=float)
    exports, audit = c3b3b1._export_history(configuration["context"], states)
    if not audit["passed"]:
        raise RuntimeError("committed h background export audit failed")
    return exports
...
def _histories(
    ...
    h_arrays = _load_npz(c3b4b1.DECISIVE_ARRAYS)
    refined_base = _load_npz(COARSE_TEMPORAL_DIRECTORY / "decisive_arrays.npz")
    base_states = _load_npz(BASE_DIRECTORY / "decisive_arrays.npz")
    h_task = c3b4b1._task_id(profile)
    h_states = h_arrays[f"{h_task}__states"]
    h_exports = h_arrays[f"{h_task}__direct_exports"]
    state_histories = [h_states - base_states[f"{LAYOUT}__states"]]
    h_response = h_exports - _h_base_exports(configuration)
    instantaneous = [h_response]
    cumulative = [c3b3b1._native_cumulative(h_response, TIMESTEP_LEVELS_SECONDS[0])]
    for timestep in REFINED_TIMESTEPS_SECONDS:
        task = _task_id(float(timestep), profile)
        base_task = c3b3b1._task_id(float(timestep), "base")
        indices = _common_indices(float(timestep))
        state_response = (
            arrays[f"{task}__states"] - refined_base[f"{base_task}__states"]
        )
        export_response = (
            arrays[f"{task}__direct_exports"]
            - refined_base[f"{base_task}__direct_exports"]
        )
        state_histories.append(state_response[indices])
        instantaneous.append(export_response[indices])
        cumulative.append(
            c3b3b1._native_cumulative(export_response, float(timestep))[indices]
        )
    return tuple(state_histories), tuple(instantaneous), tuple(cumulative)
...
def _analyze(
    ...
    arrays: dict[str, np.ndarray],
    configuration: dict,
    temporal_manifest: dict,
) -> tuple[dict, dict[str, np.ndarray]]:
    spatial_arrays = _load_npz(SPATIAL_PILOT_DIRECTORY / "decisive_arrays.npz")
    gates = c3b3b1._temporal_gates(temporal_manifest)
    field_scales = np.asarray(spatial_arrays["field_scales"], dtype=float)
    observable_scales = np.asarray(
        spatial_arrays["fixed_physical_observable_scales"], dtype=float
    )
    parent_temporal = _read_json(COARSE_TEMPORAL_DIRECTORY / "summary.json")
    reports = list(progress["trajectory_reports"].values())
    method_floor = max(
        float(parent_temporal["temporal_screen"]["numerical_uncertainty_floor"]),
    ...
    for profile in PROFILES:
        state, instantaneous, cumulative = _histories(profile, arrays, configuration)
        state_metric = c3b3b1._state_metric(
            state, configuration, field_scales, gates, method_floor
        )
        instant_metric = c3b3b1._export_metric(
            instantaneous, observable_scales, gates, method_floor
        )
        cumulative_metric = c3b3b1._export_metric(
            cumulative, observable_scales * HORIZON_SECONDS, gates, method_floor
        )
        passed = bool(
            state_metric["passed"]
        ...
        for index, label in enumerate(("h", "h2", "h4")):
            decisive[f"{profile}__{label}__state_response"] = state[index]
            decisive[f"{profile}__{label}__instantaneous_export_response"] = instantaneous[index]
            decisive[f"{profile}__{label}__cumulative_export_response"] = cumulative[index]
    return (
        {
            "passed": all_passed,
...
def _report(summary: dict) -> str:
    ...
    lines = [
        ...
        "",
        "The five prospectively frozen held-outs were compared at "
        "`dt=1e-5/5e-6/2.5e-6 s` on the coarse embedded layout through "
        "the common `4e-5 s` horizon. The committed selected-step profiles "
        "and all background histories were reused by hash.",
        "",
        "## Binding results",
        "",
    ]
    for profile, report in screen["case_reports"].items():
        state = report["state"]
        ...
        lines.extend(
            [
                ...
                "- selected-step Richardson error "
                f"(state/instantaneous/cumulative): "
    ...
    lines.extend(
        [
            ...
            "",
            "This certifies held-out temporal behavior only on the coarse "
            "layout. Middle/fine spatial confirmation, duration extension, "
...
def _package(
    ...
    classification = (
        ...
        if passed
        else "coarse_heldout_profile_temporal_refinement_failed_profile_breadth_campaign_blocked"
    )
    authorized_next = (
        "WP10c9d6c7c3b4b3_middle_fine_heldout_profile_spatial_confirmation"
        if passed
        else "WP10c9d6c7c3b4b2_temporal_failure_localization"
    )
    config = {
        "schema_version": SCHEMA_VERSION,
        "work_package": WORK_PACKAGE,
        ...
        "temporal_gates": c3b3b1._temporal_gates(temporal_manifest),
    }
    ...
    summary = {
        ...
        "coarse_heldout_temporal_convergence_certified": passed,
        "middle_fine_heldout_spatial_confirmation_authorized": passed,
    ...
    input_paths = {
        ...
        "parent_arrays": c3b4b1.DECISIVE_ARRAYS,
        "breadth_manifest": c3b4a.MANIFEST_PATH,
        "temporal_manifest": TEMPORAL_MANIFEST_DIRECTORY
        / "temporal_refinement_manifest.json",
        "background_base_arrays": BASE_DIRECTORY / "decisive_arrays.npz",
        "background_refined_arrays": COARSE_TEMPORAL_DIRECTORY
        / "decisive_arrays.npz",
    }
    _write_json(
        PROVENANCE_PATH,
...
def main() -> int:
    parent, breadth_manifest, temporal_manifest = _validate_parent()
    configuration = c3b1a._configurations()[LAYOUT]
    print(f"b4b2: build tangent {LAYOUT}", flush=True)
    ...
    print(f"b4b2: tangent built in {time.perf_counter() - started:.2f}s", flush=True)
    progress, progress_arrays = _run_refined(configuration, tangent)
    if progress["failed"]:
        screen = {
            "passed": False,
            "case_reports": {},
            ...
            "maximum_scaled_residual": max(
                ...
                for report in progress["trajectory_reports"].values()
            ),
            "maximum_discrete_ledger_defect": max(
        ...
        screen, decisive = _analyze(
            progress, progress_arrays, configuration, temporal_manifest
        )
    return _package(
        parent,
        breadth_manifest,
