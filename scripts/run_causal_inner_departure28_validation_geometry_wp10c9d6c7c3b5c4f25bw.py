#!/usr/bin/env python3
"""Execute the frozen departure-28 independent geometry set."""
...
SCHEMA_VERSION = 1
WORK_PACKAGE = "WP10c9d6c7c3b5c4f25bw"
MANIFEST_COMMIT = "e8ead0c019075113bb80a59c2b22f29975b1f3df"
MANIFEST_PARENT = "627f3aceb87fd17b59f6972c7ae60e0a9c915a7a"
MANIFEST_TREE = "f8a08279c5907d810efd154f2d94e96b2a9f0ccf"
PASS_CLASSIFICATION = "departure28_independent_validation_geometry_passed"
FAIL_CLASSIFICATION = "departure28_independent_validation_geometry_failed"
AUTHORIZED_NEXT = "WP10c9d6c7c3b5c4f25bx"

ARTIFACT = (
    "causal_inner_departure28_validation_geometry_"
    "wp10c9d6c7c3b5c4f25bw"
)
CANONICAL_DIRECTORY = ROOT / "results/canonical" / ARTIFACT
SCRATCH_DIRECTORY = ROOT / "outputs/checkpoints" / ARTIFACT
THIS_RUNNER = (
    "scripts/run_causal_inner_departure28_validation_geometry_"
    "wp10c9d6c7c3b5c4f25bw.py"
)
THIS_TEST = (
    "tests/test_causal_inner_departure28_validation_geometry_"
    "wp10c9d6c7c3b5c4f25bw.py"
)
REPORT_RELATIVE = (
    "docs/reports/current/CODEX_CAUSAL_INNER_DEPARTURE28_VALIDATION_"
    "GEOMETRY_WP10C9D6C7C3B5C4F25BW_2026-08-19.md"
)
...
def _validate_manifest(*, require_clean: bool) -> dict:
    if _git("rev-parse", MANIFEST_COMMIT) != MANIFEST_COMMIT:
        raise RuntimeError("departure-28 validation manifest commit changed")
    if _git("rev-parse", f"{MANIFEST_COMMIT}^") != MANIFEST_PARENT:
        raise RuntimeError("departure-28 validation manifest lineage changed")
    if _git("rev-parse", f"{MANIFEST_COMMIT}^{{tree}}") != MANIFEST_TREE:
        raise RuntimeError("departure-28 validation manifest tree changed")
    ...
    design_metrics = _read(manifest.CANONICAL_DIRECTORY / "design_metrics.json")
    if (
        not summary["passed"]
        ...
        raise RuntimeError("departure-28 geometry authorization changed")
    ...
    if require_clean and _git("status", "--short", "--untracked-files=no"):
        raise RuntimeError("departure-28 geometry requires a clean tracked tree")
            ...
            specifications.append(
                {
                    ...
                    "active_direction": np.asarray(directions[:, local_index], dtype=float),
...
def _run() -> dict:
    ...
    if CANONICAL_DIRECTORY.exists():
        raise RuntimeError("departure-28 validation geometry already canonicalized")
    ...
    engine._write_json(CANONICAL_DIRECTORY / "metrics.json", {"checks": checks, **metrics})
    engine._write_npz(CANONICAL_DIRECTORY / "departure28_geometry_database.npz", arrays)
    summary = {
    ...
    REPORT_PATH.write_text(
        "\n".join(
            (
                "# Departure-28 validation geometry WP10c9d6c7c3b5c4f25bw",
                "",
                "## Classification",
                "",
                f"`{classification}`",
                "",
                f"Completed `{metrics['completed_candidate_count']}` of `{manifest.PLANNED_CANDIDATES}` signed exact retractions; failures: `{metrics['failed_candidate_count']}`.",
                "",
