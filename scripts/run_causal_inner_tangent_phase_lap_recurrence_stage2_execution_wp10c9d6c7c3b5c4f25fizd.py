#!/usr/bin/env python3
"""Execute the second bounded tranche of tangent-phase-lap acquisition."""
...
import run_causal_inner_tangent_phase_lap_recurrence_stage2_manifest_wp10c9d6c7c3b5c4f25fizc as manifest  # noqa: E402
...
PASS_CLASSIFICATION = "tangent_phase_lap_recurrence_stage2_completed"
COARSE_RECURRENCE_CLASSIFICATION = (
    "coarse_tangent_phase_recurrence_candidate_observed_during_stage2"
)
OPEN_CLASSIFICATION = "tangent_phase_lap_without_coarse_state_recurrence"
PHASE_FAILURE_CLASSIFICATION = (
    "tangent_phase_lap_recurrence_stage2_geometry_failed"
)
PHYSICAL_FAILURE_CLASSIFICATION = (
    "tangent_phase_lap_recurrence_stage2_physical_gate_failed"
)
NUMERICAL_FAILURE_CLASSIFICATION = (
    "tangent_phase_lap_recurrence_stage2_numerical_or_restart_failed"
)
AUTHORIZED_NEXT = (
    "WP10c9d6c7c3b5c4f25fize_"
    "tangent_phase_lap_recurrence_stage3_manifest"
)
...
INITIAL_ELAPSED_SECONDS = 0.18000000000000013
...
MAXIMUM_ACCEPTED_SEGMENTS = manifest.STAGE_ACCEPTED_ENDPOINTS
MAXIMUM_ATTEMPTED_SEGMENTS = manifest.MAXIMUM_ATTEMPTED_ENDPOINTS
...
def _contract() -> dict:
    return _helper()._read(manifest.CANONICAL_DIRECTORY / "stage2_contract.json")


def _seed() -> dict[str, np.ndarray]:
    return _load_npz(manifest.CANONICAL_DIRECTORY / "stage2_seed.npz")
...
def _validate_manifest(*, require_clean: bool) -> dict:
    ...
    metrics = helper._read(manifest.CANONICAL_DIRECTORY / "stage2_metrics.json")
    contract = _contract()
    provenance = helper._read(manifest.CANONICAL_DIRECTORY / "provenance.json")
    scope = contract["scope"]
    gates = contract["binding_stage2_gates"]
    proof = contract["phase_lap_scope_proof"]
    if (
        summary["classification"] != manifest.CLASSIFICATION
        or not summary["passed"]
        or not summary["definitions_only"]
        or not summary["stage2_execution_authorized"]
        or summary["stage2_execution_executed"]
        or summary["phase_lap_execution_authorized"]
        ...
        or scope["prior_accepted_phase_endpoints"] != 48
        or scope["new_accepted_phase_endpoints"] != 48
        or scope["maximum_attempted_endpoints"] != 52
        or scope["maximum_exact_free_field_calls"] != 54
        or scope["maximum_retractions"] != 54
        ...
        != 10.0
        or gates["new_accepted_phase_endpoints"] != 48
        or gates["combined_accepted_phase_endpoints"] != 96
        or proof["phase_lap_reachable_within_stage2"]
    ):
        raise RuntimeError("stage2 authorization changed")
    for relative, expected in provenance["source_hashes"].items():
        if helper._sha(ROOT / relative) != expected:
            raise RuntimeError(f"stage2 manifest source changed: {relative}")
    manifest._validate_parent(require_clean=False)
    if require_clean and helper._git(
        "status", "--short", "--untracked-files=no"
    ):
        raise RuntimeError("stage2 execution requires a clean tracked tree")
    return {
        "hashes": hashes,
        "summary": summary,
        "metrics": metrics,
...
def _prepare_scratch(lock: dict) -> dict:
    ...
    if SCRATCH_DIRECTORY.exists():
        if not path.exists() or helper._read(path) != identity:
            raise RuntimeError("phase-lap stage2 scratch mismatch")
    else:
...
def _classify(
    ...
    phase_failed = any(
        not item["phase_geometry"]["passed"] for item in phase_records
    )
    candidate = any(
        item["coarse_recurrence_candidate"] for item in recurrence
    )
    phase_lap = any(item["phase_lap_observed"] for item in recurrence)
    combined = manifest.PRIOR_ACCEPTED_ENDPOINTS + len(accepted)
    initial_phase = float(seed["unwrapped_phase_advance_radians"])
    cumulative_phase = (
        initial_phase
        if final is None
        else final["cumulative_phase_advance_radians"]
    )
    new_phase_advance = cumulative_phase - initial_phase
    endpoint_fields = [
        item["endpoint_field"]
        for item in records
    ...
    all_blocks = bool(
        endpoint_fields
        and all(
            item["metric_chart"]["block_sizes"] == [442, 28]
            for item in endpoint_fields
        )
    )
    completion = bool(
        metrics["passed"]
        and len(accepted) == MAXIMUM_ACCEPTED_SEGMENTS
        and combined == manifest.COMBINED_ACCEPTED_ENDPOINTS
        and new_phase_advance >= manifest.MINIMUM_NEW_PHASE_ADVANCE
        and phase_records
    ...
    values.update(
        {
            "prior_accepted_phase_endpoints": manifest.PRIOR_ACCEPTED_ENDPOINTS,
            ...
            "initial_cumulative_phase_advance_radians": initial_phase,
            "new_cumulative_phase_advance_radians": new_phase_advance,
    ...
    result_arrays.update(
        {
            ...
            "new_phase_lap_flags": np.asarray(
                [item["phase_lap_observed"] for item in recurrence], dtype=bool
            ),
            "new_coarse_recurrence_candidate_flags": np.asarray(
                [item["coarse_recurrence_candidate"] for item in recurrence],
                dtype=bool,
            ),
        }
...
def _canonicalize(
    ...
    if CANONICAL_DIRECTORY.exists() or REPORT_PATH.exists():
        raise RuntimeError("stage2 execution result already exists")
    CANONICAL_DIRECTORY.mkdir(parents=True)
    helper._write_json(CANONICAL_DIRECTORY / "stage2_execution_metrics.json", metrics)
    _save_npz(CANONICAL_DIRECTORY / "stage2_execution_arrays.npz", arrays)
    helper._write_json(
    ...
    summary = {
        ...
        "new_cumulative_phase_advance_radians": values[
            "new_cumulative_phase_advance_radians"
        ],
    ...
    REPORT_PATH.write_text(
        "\n".join(
            (
                "# Tangent-phase-lap recurrence stage-2 execution",
                "",
                f"Classification: `{metrics['classification']}`.",
                "",
                f"Accepted `{values['new_accepted_phase_endpoints']}` of `{values['attempted_segments']}` attempted stage-2 endpoints. The combined chain contains `{values['combined_accepted_phase_endpoints']}` accepted phase endpoints and reaches `{values['terminal_elapsed_seconds']:.6f}` s.",
                "",
                f"New registered phase advance is `{values['new_cumulative_phase_advance_radians']:.9f}` rad and cumulative phase is `{values['cumulative_phase_advance_radians']:.9f}` rad. Phase lap: `{values['phase_lap_observed']}`. Coarse recurrence candidate: `{values['coarse_recurrence_candidate_observed']}`.",
                "",
                f"Maximum endpoint/blind defects are `{values['maximum_accepted_endpoint_integral_defect']}` / `{values['maximum_accepted_blind_midpoint_rate_defect']}`. Maximum raw/metric conditions are `{values['maximum_raw_coordinate_jacobian_condition']}` / `{values['maximum_metric_coordinate_jacobian_condition']}`.",
                "",
                f"All endpoint charts were 442+28: `{values['all_endpoint_metric_blocks_are_442_plus_28']}`. Checkpoint/suffix replay: `{values['all_accepted_checkpoint_roundtrips_bitwise']}` / `{values['suffix_history_replay_bitwise']}`.",
                "",
                f"Authorized next artifact: `{metrics['authorized_next']}`. This result is not a phase-lap, cycle, complete-cycle, or reduced-slow-evolution certificate.",
                "",
            )
        ),
        encoding="utf-8",
