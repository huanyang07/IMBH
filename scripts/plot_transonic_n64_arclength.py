"""Plot pseudo-arclength continuation rows for the N=64 transonic branch."""
...
TABLE_INPUT = ROOT / "outputs" / "tables" / "transonic_n64_arclength.md"
FIGURE_OUTPUT = ROOT / "outputs" / "figures" / "transonic_n64_arclength.png"
RESIDUAL_TOL = 3.0e-4
...
def main() -> None:
    ...
    draw.text((75, 28), "N=64 pseudo-arclength continuation", fill="#1f1f1f", font=header_font)
    draw.text((75, 75), "Arclength crosses the fixed-rate sonic-D wall and continues cleanly past 0.5 Eddington.", fill="#4a4a4a", font=label_font)

    bounds = (135, 165, 1210, 585)
    x_min, x_max = 0.28, 0.62
    y_min, y_max = 8.0e-5, 4.0e-4
    draw_axes(
        draw,
        bounds,
        [0.3, 0.35, 0.4, 0.5, 0.56],
        [1.0e-4, 1.5e-4, 2.0e-4, 3.0e-4, 4.0e-4],
        x_min,
        x_max,
        y_min,
        y_max,
