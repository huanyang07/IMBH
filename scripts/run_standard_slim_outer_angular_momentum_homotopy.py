"""Outer circularization angular-momentum homotopy for the standard slim disk."""
...
ANCHOR_CHECKPOINT = ROOT / os.environ.get(
    "IMBH_STANDARD_SLIM_OUTER_ANGULAR_ANCHOR",
...
TABLE_OUTPUT = ROOT / os.environ.get(
    "IMBH_STANDARD_SLIM_OUTER_ANGULAR_TABLE",
...
FIGURE_OUTPUT = ROOT / os.environ.get(
    "IMBH_STANDARD_SLIM_OUTER_ANGULAR_FIGURE",
    "outputs/figures/slim_benchmark_outer_angular_homotopy_mdot1_rout1000.png",
)
CHECKPOINT_DIR = ROOT / os.environ.get(
    "IMBH_STANDARD_SLIM_OUTER_ANGULAR_CHECKPOINTS",
    "outputs/checkpoints/slim_benchmark_outer_angular_homotopy_mdot1_rout1000",
)

SECOND_CONDITION = os.environ.get("IMBH_STANDARD_SLIM_OUTER_ANGULAR_SECOND_CONDITION", "thin_energy")
BRANCH_SPECS = tuple(
    piece.strip()
    for piece in os.environ.get(
        "IMBH_STANDARD_SLIM_OUTER_ANGULAR_BRANCHES",
        "subK:0,-0.001,-0.002,-0.005,-0.01;superK:0,0.001,0.002,0.005,0.01",
    ).split(";")
    if piece.strip()
)
NEWTON_MAX_ITER = int(os.environ.get("IMBH_STANDARD_SLIM_OUTER_ANGULAR_NEWTON_MAX_ITER", "28"))
NEWTON_MAX_NFEV = int(os.environ.get("IMBH_STANDARD_SLIM_OUTER_ANGULAR_NEWTON_MAX_NFEV", "2600"))
NEWTON_MAX_STEP_NORM = float(os.environ.get("IMBH_STANDARD_SLIM_OUTER_ANGULAR_NEWTON_MAX_STEP_NORM", "0.16"))
NEWTON_LINEAR_SOLVER = os.environ.get("IMBH_STANDARD_SLIM_OUTER_ANGULAR_NEWTON_LINEAR_SOLVER", "regularized_lsmr")
POLISH_METHOD = os.environ.get("IMBH_STANDARD_SLIM_OUTER_ANGULAR_POLISH_METHOD", "newton")
ACCEPTANCE_TOL = float(os.environ.get("IMBH_STANDARD_SLIM_OUTER_ANGULAR_ACCEPTANCE_TOL", "1e-5"))
ANCHOR_TOL = float(os.environ.get("IMBH_STANDARD_SLIM_OUTER_ANGULAR_ANCHOR_TOL", "3e-6"))
REFRESH_REPOLISH = os.environ.get("IMBH_STANDARD_SLIM_OUTER_ANGULAR_REFRESH_REPOLISH", "0") != "0"
PIVOTS = tuple(
    piece.strip()
    for piece in os.environ.get("IMBH_STANDARD_SLIM_OUTER_ANGULAR_PIVOTS", "C2,C1").replace(":", ",").split(",")
    if piece.strip()
)

if SECOND_CONDITION not in {"thin_energy", "entropy", "temperature"}:
    raise ValueError("IMBH_STANDARD_SLIM_OUTER_ANGULAR_SECOND_CONDITION must be thin_energy, entropy, or temperature")
...
def second_condition_target(z: np.ndarray, params: TransonicSlimParams, data) -> float | None:
    if SECOND_CONDITION == "thin_energy":
        return None
    if SECOND_CONDITION == "temperature":
        if "outer_temperature_logT" in data and np.isfinite(float(data["outer_temperature_logT"])):
            return float(data["outer_temperature_logT"])
        return outer_temperature_logT(z, params)
    if "outer_entropy_logK" in data and np.isfinite(float(data["outer_entropy_logK"])):
        return float(data["outer_entropy_logK"])
    return entropy_proxy(z, params)
...
def params_for(
    ...
    *,
    ...
    elif SECOND_CONDITION == "temperature":
        closure = "pressure_supported_temperature"
        outer_temperature_logT = float(second_target)
        outer_entropy_logK = None
    else:
        closure = "pressure_supported_entropy"
        outer_temperature_logT = None
        outer_entropy_logK = float(second_target)
...
def angular_diagnostic(z: np.ndarray, params: TransonicSlimParams) -> dict[str, float]:
    ...
    omega_target = float(pressure_target + params.outer_omega_log_offset)
    return {
        "pressure_target": float(pressure_target),
        "omega_target": omega_target,
        "achieved_omega_log_offset": float(ln_omega - pressure_target),
        "omega_target_residual": float(ln_omega - omega_target),
    }
...
def row_for_result(
    *,
    ...
    angular = angular_diagnostic(z, params)
    return {
        "branch": branch,
        "relative_offset": float(relative_offset),
        "omega_log_offset": float(params.outer_omega_log_offset),
        "outer_l_factor": float(np.exp(params.outer_omega_log_offset)),
...
def save_checkpoint(row: dict[str, Any], params: TransonicSlimParams) -> None:
    ...
    safe_offset = f"{float(row['omega_log_offset']):.4g}".replace(".", "p").replace("-", "m")
    stem = f"{safe_branch}_omega_offset_{safe_offset}_mdot_{float(row['ratio']):.8g}_N{int(row['N'])}".replace(".", "p")
    slopes = params.outer_match_log_slopes
    ...
    np.savez_compressed(
        ...
        second_condition=np.array(row["second_condition"]),
...
def write_table(rows: list[dict[str, Any]]) -> None:
    ...
    lines = [
        ...
        "",
        f"Anchor `{ANCHOR_CHECKPOINT.relative_to(ROOT)}`, second condition `{SECOND_CONDITION}`, "
        f"branches `{';'.join(BRANCH_SPECS)}`, refresh repolish `{REFRESH_REPOLISH}`.",
        "",
    ...
    for row in rows:
        ...
        lines.append(
            "| {branch} | {relative_offset} | {omega_log_offset} | {outer_l_factor} | {ratio} | {R_out_rg} | "
            "{initial_full} | {final_full} | {accepted} | {anchor_eligible} | {dominant} | {omega_target_residual} | "
            "{achieved_omega_log_offset} | {interval_R} | {interval_E} | {outer_energy} | {max_H_R} | "
...
def write_figure(rows: list[dict[str, Any]]) -> None:
    ...
    offsets = np.asarray([float(row["omega_log_offset"]) for row in rows], dtype=float)
    ...
    for branch in sorted(set(str(row["branch"]) for row in rows)):
        selected = sorted([row for row in rows if row["branch"] == branch], key=lambda row: float(row["omega_log_offset"]))
        points = []
        for row in selected:
            xx = float(row["omega_log_offset"])
    ...
    draw.text((90, 25), "Outer angular momentum homotopy: residual vs log(l/l_K) offset", fill=(20, 20, 20), font=font)
...
def run_branch(
    *,
    ...
    for relative_offset in offsets:
        total_offset = float(base_offset + relative_offset)
        params = params_for(
            fiducial,
            mdot_edd,
        ...
        print(f"{label} rel_offset={relative_offset:g} total_offset={total_offset:g} initial={max_residual(seed, params):.3e}", flush=True)
        ...
        print(
            ...
            f"omega_res={row['omega_target_residual']:.3e} accepted={row['accepted']} anchor={row['anchor_eligible']}",
            flush=True,
        )
        if row["accepted"]:
...
def main() -> None:
    ...
    anchor_z, anchor_params, base_offset, second_target = load_anchor(ANCHOR_CHECKPOINT, fiducial, mdot_edd)
    rows: list[dict[str, Any]] = []
