#!/usr/bin/env python3
"""Run the frozen middle-layout primary nonlinear temporal confirmation."""
...
ANALYZED_BASE_COMMIT = "f8d9b1f20eae1c90fff77d4903d0c9179f676652"
ANALYZED_BASE_PARENT = "ecab6edec3fa760655cfc7ab8b7488afe26cca39"
ANALYZED_BASE_TREE = "c39a21975c6f41dc9d6bd3c42d4e0c31591e7139"

LAYOUT = c3b3a.MIDDLE_LAYOUT
PROFILES = (c3b3a.PRIMARY_PROFILE,)
...
ARTIFACT = (
    "causal_inner_nonlinear_temporal_middle_primary_"
    "wp10c9d6c7c3b3b2"
)
THIS_RUNNER = (
    "scripts/run_causal_inner_nonlinear_temporal_middle_primary_"
    "wp10c9d6c7c3b3b2.py"
)
THIS_TEST = (
    "tests/test_causal_inner_nonlinear_temporal_middle_primary_"
    "wp10c9d6c7c3b3b2.py"
)
REPORT_RELATIVE = (
    "docs/reports/current/"
    "CODEX_CAUSAL_INNER_NONLINEAR_TEMPORAL_MIDDLE_PRIMARY_"
    "WP10C9D6C7C3B3B2_2026-08-01.md"
)
REPORT_PATH = ROOT / REPORT_RELATIVE

MANIFEST_DIRECTORY = c3b3a.CANONICAL_DIRECTORY
PARENT_SCREEN_DIRECTORY = (
    ROOT
    / "results/canonical/"
...
def _validate_parent() -> tuple[dict, dict]:
    parent_screen = _read_json(PARENT_SCREEN_DIRECTORY / "summary.json")
    manifest = _read_json(
        MANIFEST_DIRECTORY / "temporal_refinement_manifest.json"
    )
    if (
        not parent_screen["passed"]
        or not parent_screen[
            "middle_primary_temporal_confirmation_authorized"
        ]
        or parent_screen["authorized_next"]
        != "WP10c9d6c7c3b3b2_middle_primary_temporal_confirmation"
        or parent_screen["classification"]
        ...
        raise RuntimeError("c3b3b1 middle-confirmation authorization changed")
    if (
        ...
        raise RuntimeError("c3b3b2 analyzed identity changed")
    return parent_screen, manifest
...
def _trajectory(
    ...
    for index in range(step_count):
        ...
        print(
            f"c3b3b2: {trajectory_id} dt={timestep:.1e} "
...
def _export_history(context, states: np.ndarray) -> tuple[np.ndarray, dict]:
    ...
    for index, state in enumerate(states):
        ...
        print(
            f"c3b3b2: direct export {index + 1}/{len(states)} "
...
def _load_progress() -> tuple[dict, dict[str, np.ndarray]]:
    ...
    if (
        ...
        raise RuntimeError("saved c3b3b2 progress belongs to different code")
...
def _restriction_geometry():
    c7a_arrays = {
        "c7a": _load_npz(c3b2b.C7A_DIRECTORY / "decisive_arrays.npz")
    }
    parent_grid, layouts, _ = c3b2b._layouts_and_contexts(c7a_arrays)
    return parent_grid, layouts[LAYOUT]


def _restrict_state_history(states: np.ndarray, layout) -> np.ndarray:
    return np.asarray(
        [
...
def _response_histories(
    ...
    for timestep in REFINED_TIMESTEPS_SECONDS:
        ...
        state_histories.append(
            _restrict_state_history(
                case_states[indices] - base_states[indices],
                configuration["restriction_layout"],
            )
        )
...
def _report(summary: dict) -> str:
    lines = [
        "# Nonlinear middle-layout primary temporal confirmation "
        "WP10c9d6c7c3b3b2",
        "",
        "## Classification",
        "",
        ...
        "",
        "The unchanged middle embedded operator was compared at "
...
def _package(
    ...
    classification = (
        "middle_primary_nonlinear_temporal_confirmation_certified_"
        "fine_primary_confirmation_authorized"
        if passed
        else "middle_primary_nonlinear_temporal_confirmation_failed_"
        "duration_extension_blocked"
    )
    authorized_next = (
        "WP10c9d6c7c3b3b3_fine_primary_temporal_confirmation"
        if passed
        else "WP10c9d6c7c3b3b2_temporal_failure_localization"
    )
    gates = _temporal_gates(manifest)
    config = {
        "schema_version": SCHEMA_VERSION,
    ...
    summary = {
        ...
        "fine_primary_temporal_confirmation_authorized": passed,
    ...
    input_paths = {
        "parent_coarse_screen_summary": PARENT_SCREEN_DIRECTORY
        / "summary.json",
...
def main() -> int:
    ...
    parent_grid, restriction_layout = _restriction_geometry()
    configuration["parent_grid"] = parent_grid
    configuration["restriction_layout"] = restriction_layout
    print(f"c3b3b2: build tangent {LAYOUT}", flush=True)
