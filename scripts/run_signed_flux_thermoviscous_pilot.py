"""Self-consistent alpha-viscosity thermal absolute-stream pilot."""
...
OUTPUT = ROOT / "outputs/tables/signed_flux_thermoviscous_pilot.json"
...
def run() -> list[dict[str, object]]:
    ...
    for n in N_VALUES:
        ...
        for outer_mode in ("tidal_wall", "zero_torque"):
            solved = solve_signed_thermoviscous_steady(
                grid,
                mass,
                alpha=0.01,
            ...
            profile = solved.thermal.profile
            ...
            row = {
                ...
                "thermal_residual": solved.thermal.maximum_normalized_residual,
                ...
                "temperature_min": float(np.min(solved.thermal.temperature)),
                "temperature_max": float(np.max(solved.thermal.temperature)),
                ...
                "luminosity_fraction_tau_lt1": float(
                    np.sum(profile.radiative_cooling_rate_cells[thin]) / max(radiative, 1.0)
                ),
