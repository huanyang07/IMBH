...
def test_canonical_trust_kernel_is_fail_closed():
    s=json.loads((target.CANONICAL_DIRECTORY/"summary.json").read_text());m=json.loads((target.CANONICAL_DIRECTORY/"kernel_metrics.json").read_text());assert s["passed"] and s["local_nonlinear_atlas_trust_region_certified"] and not s["trajectory_authorized"];assert m["endpoint_pair_count"]==376 and m["passing_endpoint_pair_count"]==376
