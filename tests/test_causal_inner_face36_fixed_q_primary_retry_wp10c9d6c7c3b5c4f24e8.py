...
def test_c4f24e8_manifest_is_bounded_and_schur_repaired() -> None:
    ...
    assert contract["maximum_ledger_relative_defect"] == 1.0e-12
    assert contract["required_schur_solve_method"] == (
        "row_column_equilibrated_LU_refined_1"
    )
    assert not contract["may_relax_residual_or_physical_gates"]


def test_c4f24e8_result_preserves_reduction_stops() -> None:
...
def test_c4f24e8_checksum_manifests_close() -> None:
    expected = {
